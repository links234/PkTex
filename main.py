...
modeToBitsPerPixel = {'1':1, 'L':8, 'P':8, 'RGB':24, 'RGBA':32, 'CMYK':32, 'YCbCr':24, 'I':32, 'F':32}
modeToBytesPerPixel = {'1':1.0/8.0, 'L':1, 'P':1, 'RGB':3, 'RGBA':4, 'CMYK':4, 'YCbCr':3, 'I':4, 'F':4}
...
def Execute(configPath):
    ...
    with open(configPath) as configJsonFile:
        ...
        if not "global" in configJson:
            print("Error: No global field!")
            return 2

        if not "dev-screen-width" in configJson["global"]:
        ...
        if not "dev-screen-height" in configJson["global"]:
        ...
        if not "targets" in configJson:
            print("Error: no targets field!")
            return 2

        if not "folders" in configJson:
            print("Error: No folders to search for files")
            return 2

        if not "result" in configJson:
        ...
        if "tex-info" in configJson["global"]:
            for filePath, texInfo in configJson["global"]["tex-info"].iteritems():
                ...
                if filePathAbs in filesData:
                    filesData[filesPathAbs].update(texInfo)
                else:
                    print("Specified file '%s' is either not found or unsupported" % (filePath))
        ...
        for target in configJson["targets"]:
            ...
            if not "width" in target:
                print("Error: target '%s' does not have width specified" % (targetName))
                continue
            targetWidth = target["width"]
            if not "height" in target:
                print("Error: target '%s' does not have height specified" % (targetName))
                continue
            targetHeight = target["height"]
            if not "cmds" in target:
                print("Error: target '%s' does not have a cmd list specified" % (targetName))
                continue

            targetsData[targetName] = {"pixel-count-after" : 0, "pixel-count-before" : 0, "bytes-count-after" : 0, "bytes-count-before" : 0}
            for filePath, fileData in filesData.iteritems():
                for func in target["cmds"]:
                    ...
                    if not "dev-width" in fileData:
                        cmd["dev-width"] = cmd["real-width"]
                    else:
                        cmd["dev-width"] = fileData["dev-width"]

                    if not "dev-height" in fileData:
                        cmd["dev-height"] = cmd["real-height"]
                    else:
                        cmd["dev-height"] = fileData["dev-height"]

                    cmd["new-width"] = math.floor( 1.0 * cmd["dev-width"] / developmentScreenWidth * targetWidth)
                    cmd["new-height"] = math.floor( 1.0 * cmd["dev-height"] / developmentScreenHeight * targetHeight)
                    ...
                    targetsData[targetName]["pixel-count-after"] += cmd["new-width"] * cmd["new-height"]
                    targetsData[targetName]["pixel-count-before"] += cmd["real-width"] * cmd["real-height"]

                    targetsData[targetName]["bytes-count-after"] += cmd["new-width"] * cmd["new-height"] * fileData["bpp"]
                    targetsData[targetName]["bytes-count-before"] += cmd["real-width"] * cmd["real-height"] * fileData["bpp"]
        ...
        for cmd in cmds:
            ...
            mkdirFile(whereTo)
            print("Target: " + cmd["path-prefix"] + " --> " + cmd["func"] + ": " + os.path.relpath(cmd["path"], scriptPath))
        ...
        for targetName, targetData in targetsData.iteritems():
            ...
            print("Saved memory: %s (%.2f%%)" %
				...
				targetData["bytes-count-after"]) / targetData["bytes-count-before"]) )
